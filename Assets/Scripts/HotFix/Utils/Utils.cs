...
public class Utils : UnitySingleton<Utils>
{
    ...
    public void SetGridLayoutSize(RectTransform rt, bool isHorizontal, int rowOrColumnCount = 1)
    {
        ...
        if (isHorizontal)
        {
            offset.x = (Mathf.CeilToInt((float)(rt.childCount - 1) / rowOrColumnCount)) * _gridLayoutSpace;
        }
        else
        {
            offset.y = (Mathf.CeilToInt((float)(rt.childCount - 1) / rowOrColumnCount)) * _gridLayoutSpace;
        }
